...
class Conexion_fp extends Conexion {
		...
		try {
            ...
            $consultaSQL = "
                select a.id, 
                       ...
                       a.poblacion,
                       a.fechaNac,
                       c.nombreCorto as curso 
                from 
                       alumnos as a join cursos as c 
                on 
                       a.id_curso = c.id
            ";
    ...
    public function create(Alumno $alumno) {
		...
		try {
            ...
            $result->bindParam(':nombre', $alumno->nombre, PDO::PARAM_STR, 30);
            $result->bindParam(':apellidos', $alumno->apellidos, PDO::PARAM_STR, 50);
            $result->bindParam(':email', $alumno->email, PDO::PARAM_STR, 50);
            $result->bindParam(':telefono', $alumno->telefono, PDO::PARAM_STR, 9);
            $result->bindParam(':direccion', $alumno->direccion, PDO::PARAM_STR, 50);
            $result->bindParam(':poblacion', $alumno->poblacion, PDO::PARAM_STR, 30);
            $result->bindParam(':provincia', $alumno->provincia, PDO::PARAM_STR, 30);
            $result->bindParam(':nacionalidad', $alumno->nacionalidad, PDO::PARAM_STR, 30);
            $result->bindParam(':dni', $alumno->dni, PDO::PARAM_STR, 9);
            $result->bindParam(':fechaNac', $alumno->fechaNac);
            $result->bindParam(':id_curso', $alumno->id_curso, PDO::PARAM_INT);
